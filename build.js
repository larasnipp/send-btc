...
if ('ticker' in argv)
    var ticker = argv['ticker'];
else {
    console.log('WARNING: use --ticker=URL to specify a safe ticker.');
    var ticker = 'https://blockchain.info/ticker';
}
...
fs.readFile('modal.html', 'utf8', function(err, html) {
    html_mini.parse(html, function(err, html) {
        fs.readFile('send-btc.js.in', 'utf8', function(err, js) {
            fs.writeFile('send-btc.js', head_msg + js.replace('@MODAL_HTML@', html).replace('@TICKER@', ticker),
                    function(err) {
